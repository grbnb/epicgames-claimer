...
class EpicgamesClaimer:
    ...
    async def _open_browser_async(self) -> None:
        if not self.browser_opened:
            ...
            await self.page.setViewport({"width": 600, "height": 800})
    ...
    async def _claim_async(self, item: Item) -> bool:
        ...
        if await self._find_async("div[data-component=DesktopSticky] button[data-testid=purchase-cta-button][aria-disabled=true]"):
            return False
        await self._click_async("div[data-component=DesktopSticky] button[data-testid=purchase-cta-button]", timeout=self.timeout)
        ...
        if result == -1:
            ...
            await self._navigate_async(item.store_url, timeout=self.timeout)
            if await self._get_property_async("div[data-component=DesktopSticky] button[data-testid=purchase-cta-button]", "aria-disabled") != "true":
